...
st.title("Supply Chain Logistik-Dashboard (PostgreSQL)")
...
menge_pro_sorte = df.groupby("bananensorte")["menge"].sum().sort_values(ascending=False)
...
menge_pro_lager = df.groupby("lager")["menge"].sum().sort_values(ascending=False)
st.bar_chart(menge_pro_lager)

# 4. Lieferantenvergleich
st.header("4. Lieferantenvergleich")
menge_pro_lieferant = df.groupby("lieferant")["menge"].sum().sort_values(ascending=False)
...
if "datum" in df.columns:
    ...
    try:
        ...
        last_date = pd.Timestamp(menge_pro_tag.index[-1])
        forecast_index = pd.date_range(last_date + pd.Timedelta(days=1), periods=7, freq='D')
